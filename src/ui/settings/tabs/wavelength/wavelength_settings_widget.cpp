...
void WavelengthSettingsWidget::SetupUi() {
    ...
    const auto info_label = new QLabel(
        translator_->Translate("WavelengthSettingsWidget.Info",
    ...
    const auto preferred_frequency_label = new QLabel(
        ...
        this);
    preferred_frequency_label->setStyleSheet(
    ...
    const auto language_label = new QLabel(
        translator_->Translate("WavelengthSettingsWidget.LanguageLabel", "Language:"), this);
    language_label->setStyleSheet(
        "color: #00ccff; background-color: transparent; font-family: Consolas; font-size: 9pt;");

    language_combo_ = new QComboBox(this);
    language_combo_->addItem(
        translator_->Translate("WavelengthSettingsWidget.LanguageEnglish", "English"), "en");
    language_combo_->addItem(
        translator_->Translate("WavelengthSettingsWidget.LanguagePolish", "Polski"), "pl");
...
void WavelengthSettingsWidget::LoadSettings() const {
    if (!frequency_value_edit_ || !frequency_unit_combo_ || !language_combo_) {
        qWarning() << "[WAVELENGTH TAB]::loadSettings - UI elements not initialized.";
        return;
    }
    ...
    if (!ok || frequency_hz < FrequencyLimits::kMinHz) {
        qWarning() << "[WAVELENGTH TAB]::loadSettings - Invalid frequency in config:" << frequency_string <<
                ". Using default 130.0 Hz.";
    ...
    if (lang_index != -1) {
        ...
        qWarning() << "[WAVELENGTH TAB]::loadSettings - Saved language code not found in combo box:" <<
                current_lang_code << ". Selecting default.";
...
bool WavelengthSettingsWidget::ValidateFrequencyInput(double &hz) {
    ...
    if (!ok || value_double <= 0) {
        QMessageBox::warning(this,
                             ...
                             translator_->Translate("WavelengthSettingsWidget.PositiveNumber",
                                                    "Please enter a valid positive number for the frequency value.")
        );
    ...
    if (value_double > FrequencyLimits::kMaxValuePerUnit) {
        QMessageBox::warning(this,
                             translator_->Translate("WavelengthSettingsWidget.InvalidInputTitle", "Invalid Input"),
                             QString(translator_->Translate("WavelengthSettingsWidget.MaxValue",
                                                            "The maximum value for %1 is %2."))
                             .arg(unit_str)
    ...
    if (hz < FrequencyLimits::kMinHz) {
        QMessageBox::warning(this,
                             translator_->Translate("WavelengthSettingsWidget.InvalidInputTitle", "Invalid Input"),
                             QString(translator_->Translate("WavelengthSettingsWidget.MinValue",
                                                            "The minimum frequency allowed is %1 Hz."))
    ...
    if (hz > FrequencyLimits::kMaxHz) {
        QMessageBox::warning(this,
                             translator_->Translate("WavelengthSettingsWidget.InvalidInputTitle", "Invalid Input"),
                             QString(translator_->Translate("WavelengthSettingsWidget.MaxValueHz",
                                                            "The maximum frequency allowed is %1 MHz."))
                             .arg(FrequencyLimits::kMaxValuePerUnit));
        frequency_value_edit_->setFocus();
        frequency_value_edit_->selectAll();
        return false;
...
void WavelengthSettingsWidget::SaveSettings() {
    if (!frequency_value_edit_ || !frequency_unit_combo_ || !language_combo_) {
        qWarning() << "[WAVELENGTH TAB] WavelengthSettingsWidget::saveSettings - UI elements not initialized.";
        return;
    }
    ...
    if (!ValidateFrequencyInput(frequency_value_hz)) {
        qWarning() <<
                "[WAVELENGTH TAB] WavelengthSettingsWidget::saveSettings - Invalid frequency input. Settings not saved.";
        return;
    }
    ...
    if (previous_language_code != selected_language_code) {
        ...
        QMessageBox::information(this,
                                 ...
                                 translator_->Translate("Global.RestartRequiredMessage",
