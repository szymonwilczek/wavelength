...
void WavelengthChatView::OnReadyReadInput() const {
    ...
    if (!buffer.isEmpty()) {
        ...
        qDebug() <<
                "[CHAT VIEW][HOST] onReadyReadInput: Buffer was empty (read 0 bytes, or device closed/error between check and read). audio_input_ error:"
                << audio_input_->error();
    }
}
...
void WavelengthChatView::StopAudioInput() {
    if (audio_input_ && (audio_input_->state() == QAudio::ActiveState || audio_input_->state() == QAudio::IdleState)) {
        qDebug() << "[CHAT VIEW][HOST] StopAudioInput: Attempting to stop. Current audio_input_ state:"
                << audio_input_->state();
        ...
        if (input_device_) {
            const bool disconnected = disconnect(input_device_, &QIODevice::readyRead, this,
                                                 &WavelengthChatView::OnReadyReadInput);
            qDebug() << "[CHAT VIEW][HOST] StopAudioInput: Disconnected readyRead from input_device_:" << disconnected;
        }

        // 2. stop QAudioInput.
        audio_input_->stop();
        qDebug() << "[CHAT VIEW][HOST] StopAudioInput: audio_input_->stop() called. New audio_input_ state:"
                << audio_input_->state();

        // 3. reset the input_device_ pointer.
        input_device_ = nullptr;

        qDebug() << "[CHAT VIEW][HOST] Audio input stopped and input_device_ nulled.";
