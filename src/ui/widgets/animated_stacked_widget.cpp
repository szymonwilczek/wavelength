...
void AnimatedStackedWidget::prepareAnimation(const int nextIndex) const {
...
void AnimatedStackedWidget::animateFade(const int nextIndex) const {
    QWidget *currentWidget = widget(currentIndex());
    QWidget *nextWidget = widget(nextIndex);
    ...
    const auto currentEffect = new QGraphicsOpacityEffect(currentWidget);
    currentWidget->setGraphicsEffect(currentEffect);

    // Efekt przezroczystości dla następnego widgetu
    const auto nextEffect = new QGraphicsOpacityEffect(nextWidget);
    nextWidget->setGraphicsEffect(nextEffect);
    nextEffect->setOpacity(0);

    // Animacja zanikania bieżącego widgetu
    const auto fadeOutAnimation = new QPropertyAnimation(currentEffect, "opacity");
    ...
    const auto fadeInAnimation = new QPropertyAnimation(nextEffect, "opacity");
...
void AnimatedStackedWidget::animateSlide(const int nextIndex) const {
    QWidget *currentWidget = widget(currentIndex());
    QWidget *nextWidget = widget(nextIndex);

    // Pozycje początkowe
    const int width = currentWidget->width();
    ...
    const auto currentAnimation = new QPropertyAnimation(currentWidget, "geometry");
    ...
    const auto nextAnimation = new QPropertyAnimation(nextWidget, "geometry");
...
void AnimatedStackedWidget::animateSlideAndFade(const int nextIndex) const {
    QWidget *currentWidget = widget(currentIndex());
    QWidget *nextWidget = widget(nextIndex);
    ...
    const auto currentMoveAnimation = new QPropertyAnimation(currentWidget, "geometry");
    ...
    const auto currentFadeAnimation = new QPropertyAnimation(currentEffect, "opacity");
    ...
    const auto nextMoveAnimation = new QPropertyAnimation(nextWidget, "geometry");
    ...
    const auto nextFadeAnimation = new QPropertyAnimation(nextEffect, "opacity");
...
void AnimatedStackedWidget::animatePush(const int nextIndex) const {
    QWidget *currentWidget = widget(currentIndex());
    QWidget *nextWidget = widget(nextIndex);
