...
JoinWavelengthDialog::JoinWavelengthDialog(QWidget *parent): AnimatedDialog(parent, kDigitalMaterialization),
    ...
    const auto title_label = new QLabel(translator_->Translate("JoinWavelengthDialog.Header", "JOIN WAVELENGTH"), this);
    ...
    const auto time_label = new QLabel(QString("TS: %1").arg(timestamp), this);
    ...
    const auto info_label = new QLabel(
        translator_->Translate("JoinWavelengthDialog.InfoLabel", "Enter the wavelength frequency (130Hz - 180MHz)"),
        this);
    ...
    const auto frequency_title_label = new QLabel(
        translator_->Translate("JoinWavelengthDialog.FrequencyLabel", "FREQUENCY:"),
        this);
    frequency_title_label->setStyleSheet(
        "color: #00ccff; background-color: transparent; font-family: Consolas; font-size: 9pt;");

    const auto freq_container = new QWidget(this);
    const auto freq_layout = new QHBoxLayout(freq_container);
    freq_layout->setContentsMargins(0, 0, 0, 0);
    freq_layout->setSpacing(5);

    frequency_edit_ = new CyberLineEdit(this);
    const auto validator = new QDoubleValidator(130, 180000000.0, 1, this);
    QLocale locale(QLocale::English);
    locale.setNumberOptions(QLocale::RejectGroupSeparator);
    validator->setLocale(locale);
    frequency_edit_->setValidator(validator);
    frequency_edit_->setPlaceholderText(
        translator_->Translate("JoinWavelengthDialog.FrequencyEdit", "ENTER FREQUENCY"));
    ...
    const auto decimal_hint_label = new QLabel(
        translator_->Translate("JoinWavelengthDialog.DecimalHintLabel", "Use dot (.) as decimal separator (e.g. 98.7)"),
        this);
    decimal_hint_label->setStyleSheet(
        "color: #008888; background-color: transparent; font-family: Consolas; font-size: 8pt;");
    form_layout->addRow("", decimal_hint_label);

    const auto password_label = new QLabel(
        translator_->Translate("JoinWavelengthDialog.PasswordLabel", "PASSWORD:"),
        this);
    ...
    password_edit_->setPlaceholderText(
        translator_->Translate("JoinWavelengthDialog.PasswordEdit", "WAVELENGTH PASSWORD (IF REQUIRED))"));
    ...
    join_button_ = new CyberButton(
        translator_->Translate("JoinWavelengthDialog.JoinButton", "JOIN WAVELENGTH"), this,
        true);
    join_button_->setFixedHeight(35);

    cancel_button_ = new CyberButton(
        translator_->Translate("JoinWavelengthDialog.CancelButton", "CANCEL"), this,
        false);
    ...
    const WavelengthJoiner *joiner = WavelengthJoiner::GetInstance();
...
void JoinWavelengthDialog::OnConnectionError(const QString &error_message) {
    ...
    if (error_message.contains("Password required")) {
        display_message = translator_->Translate("JoinWavelengthDialog.PasswordRequired",
                                                 "WAVELENGTH PASSWORD PROTECTED");
        ...
        display_message = translator_->
                Translate("JoinWavelengthDialog.WavelengthUnavailable", "WAVELENGTH UNAVAILABLE");
    }
