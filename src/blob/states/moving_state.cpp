...
void MovingState::apply(std::vector<QPointF>& controlPoints, 
    ...
    if (std::abs(avgVelX) > 0.1 || std::abs(avgVelY) > 0.1) {
        ...
        for (size_t i = 0; i < controlPoints.size(); ++i) {
            ...
            auto stretchForce = QPointF(avgVelX * factor, avgVelY * factor);
...
void MovingState::applyInertiaForce(std::vector<QPointF>& velocity,
    ...
    const double windowSpeed = windowVelocity.length();
    if (windowSpeed < 0.1) {
        return;
    }

    // Zmniejsz współczynnik siły bezwładności z 0.005 na 0.0025
    const QVector2D inertiaForce = -windowVelocity * (0.0025 * qMin(1.0, windowSpeed / 10.0));

    // Prekalkujemy dla wydajności
    const double forceX = inertiaForce.x();
    const double forceY = inertiaForce.y();
    ...
    const double invRadius = 1.0 / blobRadius;
    ...
    for (size_t i = 0; i < controlPoints.size(); ++i) {
        ...
        const double distApprox = qAbs(distX) + qAbs(distY);
        const double forceScale = qMin(distApprox * invRadius * 0.2, 0.3); // Zmniejszono oba współczynniki
    ...
    const double centerFactor = 0.0003 * qMin(1.0, windowSpeed / 5.0);
    ...
    if (windowSpeed > 5.0) {
        for (size_t i = 0; i < controlPoints.size(); ++i) {
            const double distX = controlPoints[i].x() - centerX;
            const double distY = controlPoints[i].y() - centerY;

            const double perpX = -forceY;
            const double perpY = forceX;

            const double dotProduct = distX * perpX + distY * perpY;
...
void MovingState::applyForce(const QVector2D& force,
    ...
    const QPointF forceXY(force.x(), force.y());
    const QPointF centerForce = forceXY * 0.2;
    blobCenter += centerForce;

    for (size_t i = 0; i < numPoints; ++i) {
        const double forceScale = qMin(distanceCache[i] / blobRadius, 1.0);
