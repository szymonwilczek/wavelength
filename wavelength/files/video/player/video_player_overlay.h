...
protected:
    void paintEvent(QPaintEvent* event) override {
        ...
        // Rysowanie wizualizacji dźwięku - małe słupki pulsujące
        int barCount = 8;
        int maxBarHeight = 12;
        int barWidth = 2;
        int barSpacing = 6;
...
protected:
    void paintEvent(QPaintEvent* event) override {
        ...
        // Lewy górny - ID
        painter.drawText(10, 15, QString("ID:%1").arg(QRandomGenerator::global()->bounded(1000, 9999)));

        // Prawy górny - timestamp
        QString timestamp = QDateTime::currentDateTime().toString("HH:mm:ss");
        painter.drawText(width() - 80, 15, QString("TS:%1").arg(timestamp));
